...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # W1 = (D, H), b1 = (H,)
    # maximum works as ReLU activation function
    Output1 = np.maximum(X.dot(W1) + b1, 0)
    ...
    # Refer to ./softmax.py
    # Course note http://cs231n.github.io/neural-networks-case-study/#grad
    softmax[(range(N), y)] -= 1
    grads['W2'] = (Output1.T.dot(softmax) / N) + (2 * reg * W2)
    grads['b2'] = (np.sum(softmax, axis=0) / N)

    # max gate is a gradient router in backpropagation
    # Refer to lecture note 4 page 50
  ...
  def predict(self, X):
    ...
    final_score = hidden.dot(self.params['W2']) + self.params['b2']
